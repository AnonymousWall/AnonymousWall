...
@DisplayName("UserController Internships Tests")
class UserControllerInternshipsTest {

    private static final String BASE_PATH = "/api/v1/users/me/internships";
    ...
    @Nested
    @DisplayName("User Own Internships - Basic Functionality")
    class BasicFunctionalityTests {

        @Test
        @DisplayName("Should return user's own internships")
        void shouldReturnUserOwnInternships() {
            internshipService.createInternship(new CreateInternshipRequest("Google", "SWE Intern"), testUser1.getId());
            internshipService.createInternship(new CreateInternshipRequest("Meta", "PM Intern"), testUser1.getId());
            internshipService.createInternship(new CreateInternshipRequest("Amazon", "SDE Intern"), testUser2.getId());
            ...
            assertNotNull(data);
            assertEquals(2, data.size(), "User1 should have 2 internships");

            for (Map internship : data) {
                Map author = (Map) internship.get("author");
                assertEquals(testUser1.getId().toString(), author.get("id"));
            }
        }

        @Test
        @DisplayName("Should return empty list when user has no internships")
        ...
        @Test
        @DisplayName("Should only return user's own internships, not others")
        void shouldOnlyReturnOwnInternships() {
            for (int i = 0; i < 3; i++) {
                internshipService.createInternship(
                    new CreateInternshipRequest("Company1-" + i, "Role1-" + i), testUser1.getId());
            }
            for (int i = 0; i < 5; i++) {
                internshipService.createInternship(
                    new CreateInternshipRequest("Company2-" + i, "Role2-" + i), testUser2.getId());
            }
    ...
    @Nested
    @DisplayName("User Own Internships - Pagination")
    class PaginationTests {

        @Test
        @DisplayName("Should paginate internships correctly")
        void shouldPaginateInternshipsCorrectly() {
            for (int i = 0; i < 25; i++) {
                internshipService.createInternship(
                    new CreateInternshipRequest("Company " + i, "Role " + i), testUser1.getId());
            }
    ...
    @Nested
    @DisplayName("User Own Internships - Sorting")
    class SortingTests {

        @Test
        @DisplayName("Should sort internships by newest first (default)")
        void shouldSortByNewestFirst() throws InterruptedException {
            ...
            assertEquals("Company 3", data.get(0).get("company"));
            assertEquals("Company 2", data.get(1).get("company"));
            assertEquals("Company 1", data.get(2).get("company"));
        }

        @Test
        @DisplayName("Should sort internships by oldest first")
        void shouldSortByOldestFirst() throws InterruptedException {
            ...
            assertEquals("Company 1", data.get(0).get("company"));
            assertEquals("Company 2", data.get(1).get("company"));
            assertEquals("Company 3", data.get(2).get("company"));
        }
    }

    @Nested
    @DisplayName("User Own Internships - Hidden Internships")
    class HiddenInternshipsTests {

        @Test
        @DisplayName("Should exclude hidden internships")
        void shouldExcludeHiddenInternships() {
            internshipService.createInternship(new CreateInternshipRequest("Google", "SWE Intern"), testUser1.getId());
            var internship2 = internshipService.createInternship(
                new CreateInternshipRequest("Meta", "PM Intern"), testUser1.getId());
            internshipService.createInternship(new CreateInternshipRequest("Amazon", "SDE Intern"), testUser1.getId());

            internshipService.hideInternship(internship2.getId(), testUser1.getId());
            ...
            Map<String, Object> body = response.body();
            List<Map> data = (List<Map>) body.get("data");
            assertEquals(2, data.size(), "Should only return non-hidden internships");
    ...
    @Nested
    @DisplayName("User Own Internships - Edge Cases")
