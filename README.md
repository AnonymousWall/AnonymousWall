...
### User Own Comments Optimization (February 2026)

**Summary:** Added optimized endpoint to retrieve a user's own comments with pagination and sorting, solving the N+1 query problem.

**New Endpoint:**
- ✅ `GET /api/v1/users/me/comments` - Get authenticated user's own comments

**What Changed:**

#### 1. Database Optimization
- Added composite index `idx_comments_user_created` on `(user_id, is_hidden, created_at DESC)`
- Enables efficient retrieval with O(log K) complexity where K = user's comment count
- Previous approach required O(N × M) queries where N = posts, M = comments per post
...
- `sort` (default: NEWEST) - NEWEST or OLDEST
  ...
  "data": [
    {
      ...
      "postId": "uuid",
      "text": "Comment text",
      "author": { "id": "uuid", "profileName": "Anonymous", "isAnonymous": true },
...
- **Before:** N+1 queries (query all posts, then comments for each post)
- **After:** Single optimized query using composite index
- **Query Complexity:** O(N × M) → O(log K)
- **Tested:** 100 comments across multiple posts retrieved in < 5 seconds

#### 4. Implementation Details
- **Repository:** `CommentRepository.java` - Added `findByUserIdAndHiddenFalse*` methods
- **Service:** `CommentsServiceImpl.java` - Added `getUserOwnComments()` method
- **Controller:** `UserController.java` - New controller for user-scoped endpoints
- **Database:** `01-schema.xml` - Added composite index via Liquibase migration
- **API Spec:** `api.yml` - Added `/users/me/comments` endpoint definition

**Notes:**
- Hidden (soft-deleted) comments are automatically excluded
- Users can only see their own comments
- Single optimized database query instead of N+1 queries
